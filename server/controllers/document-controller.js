...
document.post("/add", async(req, res) => {
    try {
        ...
        await pool.query(insertDocQuery, [doc_title, doc_content, currentTime, currentTime, user_id])
        res.status(200).send({
...
document.put("/:id", async(req, res) => {
    try {
        ...
        `
        await pool.query(updateDocumentQuery, [doc_title, doc_content, id, user_id])
